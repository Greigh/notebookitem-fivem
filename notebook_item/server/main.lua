...
-- Handle content update from client (notebook items only)
RegisterNetEvent('notebook_item:updateContent', function(slot, content, itemType)
    ...
    -- Validate that this is a notebook item type only
    local allowedNotebookItems = { 'notebook', 'journal', 'businesscard', 'photo' }
    local isValidNotebookItem = false
    for _, allowedItem in ipairs(allowedNotebookItems) do
        if itemType == allowedItem then
            isValidNotebookItem = true
            break
        end
    end

    if not isValidNotebookItem then
        print(('[notebook_item] Rejected non-notebook item: %s'):format(itemType))
        TriggerClientEvent('notebook_item:contentUpdated', src, false, 'This resource only handles notebook items')
        return
    end
    ...
    if not itemConfig then
        TriggerClientEvent('notebook_item:contentUpdated', src, false, 'Invalid notebook item type')
        return
    end
    ...
    if #content > itemConfig.maxLength then
        TriggerClientEvent('notebook_item:contentUpdated', src, false,
            'Content is too long (max ' .. itemConfig.maxLength .. ' characters)')
        return
    end
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        print(('[notebook_item] Notebook Items Resource started successfully'))
        print(('[notebook_item] Supported notebook items: %s'):format(table.concat({
            'notebook', 'journal', 'businesscard', 'photo'
        }, ', ')))
        print(('[notebook_item] This resource only handles notebook items'))
    end
end)
...
-- Export function to get configuration (for phone app and other resources)
